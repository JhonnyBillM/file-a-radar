...
class DeviceViewController: UIViewController {
  ...
  /// Keep track of the updated position to manually update the cell
  /// since DiffableDataSource does not update the cell when the FetchedResultsController updates an object.
...
extension DeviceViewController: NSFetchedResultsControllerDelegate {
  func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChangeContentWith snapshot: NSDiffableDataSourceSnapshotReference) {
    dataSource.apply(snapshot as NSDiffableDataSourceSnapshot<NSString, NSManagedObjectID>, animatingDifferences: true) { [weak self] in
      
      // MARK: - Workaround
      // DiffableDataSource has a bug where it does not updates a cell
      // when the FetchedResultsController updates an object (an update that is not an insert/delete/move operation).
      //
      // What we can do is to manually update the cell at the updated position after the snapshot is applied.
      
      guard let selectedIndexPath = self?.selectedIndexPath else { return }
      guard let cell = self?.collectionView.cellForItem(at: selectedIndexPath) as? DeviceCollectionViewCell else { return }
      guard let device = self?.fetchResultsController.object(at: selectedIndexPath) else { return }
